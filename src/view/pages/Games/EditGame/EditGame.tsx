...
const EditGame: FC = () => {
    const game = useSelector((state: RootState) => state.globalStates.selectedCard) as Game;
    ...
    useEffect(() => {
        ...
        if (updatedGame) {
            ...
            const deleted = (game.units || []).filter(u => !newUnits.some(nu => nu.unitID === u.unitID)).map(u => u.unitID);
