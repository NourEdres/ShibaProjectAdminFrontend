...
import { ObjectLocation, Location, ObjectImage } from '../../../../redux/models/Interfaces';
...
const EditObjectLocation: React.FC = () => {
    ...
    const [objectDescription, setObjectDescription] = useState(object.description || '');
    const [objectImages, setObjectImages] = useState<ObjectImage[]>(object.objectImages || []);
    ...
    const handleSaveObject = async () => {
        ...
        formData.append('object', new Blob([JSON.stringify(updatedObject)], { type: 'application/json' }));
    ...
    return (
        <div className='main-container-edit-object'>
            <Loader isLoading={isLoading} message={loadingMessage} />
            <div className='edit-object-header'>
                <div className='arrow-icon'>
                    <img className='arrow-icon' src={LeftArrowIcon} alt='arrow' />
                </div>
            </div>
            <div className='edit-object-container' dir="rtl">
                <div className='edit-object-title'>{EditObjectHebrew.EditObject}</div>
                <div className='input-group'>
                    <label className='input-label'>{EditObjectHebrew.Name}</label>
                    <input type='text' className='object-input' value={objectName} onChange={e => setObjectName(e.target.value)} />
                </div>
                <div className='input-group'>
                    <label className='input-label'>{EditObjectHebrew.Description}</label>
                    <textarea className='object-textarea' value={objectDescription} onChange={e => setObjectDescription(e.target.value)} ></textarea>
                </div>

                <div className='options-container'>
                    <div className="option-section">
                        <label htmlFor="file-upload" className="input-label">{EditObjectHebrew.UploadImages}</label>
                    ...
                    <div className='object-media-list'>
                        <div className="image-count">{EditObjectHebrew.ImagesNumber} {objectImages.length + newFiles.length}</div>
                            ...
                            <Swiper {...SwiperConfig}>
                                {objectImages.map((img: ObjectImage) => (
                                    <SwiperSlide key={img.id} className='swiper-slide'>
                                        <img className='img-media'
                                            ...
                                            alt={img.name}
                                        />
                                        <button className='delete-image-btn' onClick={() => handleDeleteExistingImage(img.id)}>
                                ...
                                {newFiles.map((file, index) => (
                                    <SwiperSlide key={`new-${index}`} className='swiper-slide'>
                                        <img className='img-media' src={URL.createObjectURL(file)} alt={`New ${index}`} />
                                        <button className='delete-image-btn' onClick={() => handleDeleteNewImage(index)}>
                                            {EditObjectHebrew.Delete_Image}
                                        </button>
                                    </SwiperSlide>
                                ))}
                ...
                <button onClick={handleSaveObject} className='save-object-button'>{EditObjectHebrew.Save}</button>
            </div>
        </div>
    );
};
